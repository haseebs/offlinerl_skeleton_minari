...
class HeavyTailedBetaGaussian(Distribution):
    ...
    def rsample(self, sample_shape):
        ...
        """
        event_shape = self.loc.shape[0]
        batch_shape = sample_shape[0]
        shape = (batch_shape, event_shape, 2)
        ...
        sqrt_lnq = torch.sqrt(-2 * self._log_q(u[:, :, 0], q=q))
        u_2pi = 2 * torch.pi * u[:, :, 1]

        z1 = sqrt_lnq * torch.cos(u_2pi)
        z2 = sqrt_lnq * torch.sin(u_2pi)

        z1 = z1.permute(1, 0)
        z2 = z2.permute(1, 0)
        
        """use either of z1, z2"""
        sample = self.loc + torch.sqrt(self.scale) * z2
        # sample = self.loc + torch.sqrt(self.scale) * z1
        # sample = self.loc + self.scale * z1
        if (batch_shape != 1 and event_shape != 1) or (batch_shape == 1 and event_shape == 1):
