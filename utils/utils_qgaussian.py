...
class MultivariateBetaGaussian(Distribution):
    ...
    def _a(self):
        ...
        return torch.sqrt(-2 * self._tau.unsqueeze(-1) * self.scale)
    ...
    def rsample(self, sample_shape):
        ...
        if torch.any(torch.isnan(scaled_Z)):
            print("nan in MVBG rsample!")
            from IPython import embed; embed()
            raise RuntimeError("MVBG runtime nan sampling")
...
class UnivariateBetaGaussian(Distribution):
    arg_constraints = {
    ...
    def __init__(self, loc, scale, alpha=2, validate_args=None):

        if isinstance(alpha, numbers.Number):
            alpha = loc.new_tensor(alpha)
        
        batch_shape = torch.broadcast_shapes(scale.shape[:-1], loc.shape[:-1], alpha.shape)
        event_shape = loc.shape[-1:]
        ...
        self._R = torch.exp(self.log_radius)

        self._sigma_sq = scale ** 2
    ...
    def _compute_tau(self):
        ...
        return (-(self._R**2)/2 * self._sigma_sq ** (-(self.alpha-1) / (self.alpha+1)))
    ...
    def _sigma_sq_from_a(self, a):
        return (a / self._R) ** (self.alpha+1)

    def _sigma_sq_from_variance(self, variance):
        return ((1 + 2*self.alpha/(self.alpha-1)) / (self._R**2) * variance) ** ((self.alpha + 1)/2)


    def pdf(self, x):
        return torch.maximum(torch.FloatTensor([0.]), (self.alpha-1)*(-self._tau - .5*(x-self.loc)**2/self._sigma_sq))**(1/(self.alpha-1))
    ...
    def rsample(self, sample_shape):
        ...
        a_m1 = self.alpha - 1
        alpha_ratio = self.alpha / a_m1

        u = torch.randn(sample_shape[0], 1)
        norm = u.norm(dim=-1).unsqueeze(dim=-1)
        u /= norm

        # Sample radius.
        # ratio = r^2 / radius^2, so r = radius * sqrt(ratio).
        ratio_dist = Beta(1/2, alpha_ratio).expand(sample_shape)
        ratio = ratio_dist.rsample()
        radius = torch.exp(self.log_radius)
        # ratio = np.random.beta(rank / 2, alpha / a_m1, size=size)
        # radius = _radius(rank, alpha)
        
        r = radius * torch.sqrt(ratio)
        z = r[:, np.newaxis] * u

        Uz = z * torch.sqrt(self._sigma_sq)
        Uz *= torch.exp(-torch.log(self._sigma_sq) / (2 * 1 + 4 / a_m1))
